...
function App() {
  ...
  const airdropSol = async () => {
    if (newWallet !== undefined) {
      try {
       // const connection = new Connection(clusterApiUrl("http://127.0.0.1:8899"), "confirmed");
       //const { Connection } = require("@solana/web3.js");

// Connect to a local Solana cluster (localnet)
const connection = new Connection("http://127.0.0.1:8899", "confirmed");

console.log("Connected to Solana localnet:", connection); 
  ...
  return (
    <div className="App">
      ...
      <header className="App-header">
        ...
        {newWallet && !airdropCompleted && (
          <h4>Requesting 2 SOL. It might take a minute...</h4>
        )}
        {provider &&
          !walletKey &&
          airdropCompleted && <h2>Connect to Phantom Wallet</h2> && (
            <button
              style={{
                fontSize: "16px",
                padding: "15px",
        ...
        {provider &&
          walletKey && (
            <p>Connected to account : {walletKey.toString()} </p>
          ) && (
            <button
              style={{
                fontSize: "16px",
